...
description: "Open-source first AI inference skill for the Morpheus decentralized network. Routes inference through GLM-5 by default with Claude as fallback. Use when setting up decentralized AI inference, configuring proxy-router and API Gateway, managing MOR token staking, or enabling hardware-aware Ollama fallback. Includes multi-key auth rotation with DIEM balance monitoring, Gateway Guardian v5 health probes, three-shift cyclic execution engine, bundled security skills, zero-dependency wallet management via macOS Keychain, x402 agent-to-agent USDC payments, and ERC-8004 registry integration."
